...
def get_logs(id, line_to_keep=200, mode="cli"):
    ...
    for step, value in l.items():
        for method, data in value.items():
            if mode == "cli":
                logs.append(Fore.GREEN + "### Step: " + step +
                            ", Method: " + method + Style.RESET_ALL)
            else:
    ...
    if mode == "cli":
        line_kept.insert(0, Fore.RED + "Keeping only last " +
                         str(line_to_keep) + " lines..." + Style.RESET_ALL)
    else:
        # line_kept.insert(0, '<span class="k">' + "Keeping only last " + str(line_to_keep) + " lines..." + "</span>")
        line_kept.insert(0, "Keeping only last " +
                         str(line_to_keep) + " lines...")
    joined_logs = "\n".join(line_kept)
    return joined_logs
