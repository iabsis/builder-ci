...
def send_matrix_notification(build):
    ...
    try:
        ...
        from notification.models import UserMatrixProfile, MatrixInfo
        import threading
        import asyncio
        ...
        # Check if user has Matrix profile and notifications enabled
        try:
            matrix_profile = UserMatrixProfile.objects.get(user=user)
        except UserMatrixProfile.DoesNotExist:
            logger.debug(f"User {user.username} has no Matrix profile configured, skipping notification")
            return

        if not matrix_profile.matrix_notifications_enabled:
            logger.debug(f"Matrix notifications disabled for user {user.username}, skipping notification")
            return

        if not matrix_profile.matrix_account:
            logger.debug(f"No Matrix account configured for user {user.username}, skipping notification")
            return
        ...
        try:
            ...
            # Skip URL if reverse fails
            pass

        # Get or create MatrixInfo for room management
        matrix_info, _ = MatrixInfo.objects.get_or_create(user=user)

        # Use the full Matrix account (@username:domain format)
        matrix_account = matrix_profile.matrix_account
        ...
        def send_matrix_in_thread():
            ...
            new_room_id = asyncio.run(send_message_with_room_update(matrix_account, matrix_info.room_id, message))
        ...
        logger.info(f"Matrix notification sent to {matrix_profile.matrix_account} for build {build.name}")
